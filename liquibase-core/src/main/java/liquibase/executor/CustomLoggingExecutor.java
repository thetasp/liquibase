...
/**
 * Custom {@link LoggingExecutor} class that exposed output attrbutes for public to access
 *
 * @author PJH
 * @version 1.0
 */
@LiquibaseService(skip = true)
public class CustomLoggingExecutor extends AbstractExecutor {

    protected Writer output;

    protected Executor delegatedReadExecutor;

    public CustomLoggingExecutor(Executor delegatedExecutor, Database database) {
    ...
    /**
     * See {@link #output}. Differences between Liquibase and Theta Implementation
     *
    ...
    private void outputStatement(SqlStatement sql, List<SqlVisitor> sqlVisitors)
        ...
        try {
            // Differences between Liquibase and Theta Implementation
            // Added to avoid other java class invoked the logging executor to output some message which
            // can be ignored or removed on SQL files.
            // Start --
            if (output == null) {
                return;
            }
            ...
            if (SqlGeneratorFactory.getInstance().generateStatementsVolatile(sql, database)) {
                throw new DatabaseException(
                        sql.getClass().getSimpleName() + " requires access to up to date database "
                                + "metadata which is not available in SQL output mode");
            }
            if (sql instanceof ExecutablePreparedStatement) {
                output.write(
                        "WARNING: This statement uses a prepared statement which cannot be execute directly "
    ...
    public <T> T queryForObject(SqlStatement sql, Class<T> requiredType,
                                List<SqlVisitor> sqlVisitors) throws DatabaseException {
